...
\section{Implementation and architecture}
	...
	\subsection{Quantum mechanics background}
		...
		We exclude the identity so that the Hamiltonian is traceless, which we can do because it corresponds to choosing a physically meaningless energy zero point.

		In the spin-one case there is no standard basis of operators $ \mathcal{A}_j $.
		Choices include the Gell-Mann matrices\cite{gell-mann_symmetries_1962}, and multiple dipole-quadrupole bases\cite{hamley_spin-nematic_2012}\cite{di_dipolequadrupole_2010}.
	...
	\subsection{Unitary time evolution and the Magnus expansion}
		...
		A Magnus series over a time step $ T $ will be guaranteed to converge only if $ \int_{t_0}^{t_0 + T}\mathrm{d}t_1\|H(t_1)\| < \pi $\cite{blanes_magnus_2009}.
		...
		Time stepping can be made from the fact that time evolution operators can be split into a product via
		\begin{align}
		...
		Some of these techniques use Gauss-Legendre quadrature sampling and Baker–Campbell–Hausdorff formula to respectively avoid the integration and commutators of the Hamiltonian, producing simple expressions that can be used for unitary time stepping\cite{blanes_fourth-_2006}.
	...
	\subsection{Lie-Trotter based Exponentiator}
		...
		We choose $ n $ to be of the form $ n = 2^\tau $, as powers of two are particularly effient large numbers to raise matrices to.
		This can be done just by iterative squaring, which requires a small number of operations.
		While there are some loose lower bounds for the number of squares $ \tau $ \cite{suzuki_generalized_1976}, using these to choose a value can cause floating point errors from over-squaring.
		...
		We found that the error minimised after a number of squares of $ \tau = 24 $.
		...
		This is a consequence of the diagonals of the matrices being close to 1, with the limited precision of floating point numbers being unable to represent these differences accurately.
		Errors appear in two forms.
		One is that iterative squaring of matrices close to the identity produces floating point cancellation errors.
		This can be fixed by only working with the difference of the matrices from the identity when squaring.
		That is, if $ A = I + a $, then instead of calculating the square $ S = I + s = A^2 $, we find $ s = (a + 2I)a $.
		The other is that if we use this method, we must calculate $ T - I $ accurately, rather than $ T $, which cannot be done by simply subtracting the identity from Equation \eqref{eq:lie_trotter_4}.
		This can be fixed by replacing the exponentials and trigonometric functions along the diagonal with specialised functions such as $ \mathrm{expm}1(x) = \exp(x) - 1 $, designed for this purpose\cite{hewlett-packard_hp_1994}.
	...
	\subsection{Discretisation and parallelisation}
		...
		\emph{spinsim} splits the full simulation into small time intervals $ [t_{k - 1}, t_{k}] $, and calculates time-evolution operators $ \mathcal{U}_k $ for these intervals massively in parallel on a GPU. % check phrase "massive"
		...
		To choose an appropriate value of $ \mathrm{d}t $, users can use the accuracy plots in Figures \ref{fig:benchmark_spin_one_step_error} and \ref{fig:benchmark_spin_half_step_error}.
		...
		We found that a the 10GiB \emph{GeForce RTX 3080} could run at full memory capacity with 300 million sample points for spin-one mode and 600 million sample points for spin-half.
	...
	\subsection{Dynamically rotating frame}
		Hamiltonians which have dominant and slowly-varying terms induce rotations around a primary axis, which is usually chosen to be the quantisation axis $ z $.
		...
		This almost always done to enable the Rotating Wave Approximation (RWA), which is an assumption that the oscillatory components of $ H_r(t) $ on average make minor contributions to time evolution, and can be ignored.
...
% \subsection{Software architecture}
	\subsection{Integrator architecture}
		\begin{figure}[h!]
			\centering
			\includegraphics[scale=0.475]{architecture.pdf}
		...
		Finally, $ u^k_l $ is premultiplied to $ \mathcal{U}^r_k $ (which is initialised to the identity), and the loop continues.
	...
	\subsection{Compilation of integrator}
		...
		In particular, this allows for compatibility with products running \emph{Apple}'s \emph{MacOS}, which are incompatible with \emph{Cuda} devices.
...
\section{Quality control}
	\subsection{Evaluation of accuracy}
		...
		Errors bellow $ 10^{-11} $ were also excluded, as this was the order of magnitude of the errors in the reference simulation.
	...
	\subsection{Comparison to alternatives}
		Using Equation \eqref{eq:error}, we ran the same error and execution time benchmarks on some alternative packages to compare \emph{spinsim}'s performance to theirs.
		...
		\begin{table}[h!]
			...
			\end{tabular}
		\end{table}
		In each case, the step sizes of the alternative integrators were limited to a maximum value to obtain simulation results of different accuracies, and the maximum number of steps were modified in some cases to allow a larger number of steps to take place.
		...
		Similarly to with the internal \emph{spinsim} benchmarks, the expected spin projection was evaluated in each case, but only the states were compared to calculate a relative error.
		...
		Regardless, for a fair comparison, both \emph{Mathematica} and \emph{SciPy} results are plotted with an artificial reduction in execution time by a factor of 8, which is an upper bound for the speed increase that could be obtained by running them parallel on an 8 core processor, which appears as dotted lines.
		Simulations from \emph{spinsim} and \emph{QuTip} automatically run multithreaded, so this handicap is not plotted for these packages.
	...
	\subsection{Parallelisation performance}
		...
		Speed benchmarks were performed using \texttt{neural-sense.sim.benchmark}, by comparing evaluation speed of typical spin-one sensing experiments on different devices.
		...
		Another factor not shown in the plot is the fact that, in practice, running highly code parallel code on a CPU on a personal computer will severely limit the responsiveness of other applications, as it can utilise the entire CPU (as it should).
