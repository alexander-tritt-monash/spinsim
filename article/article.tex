...
\section{Implementation and architecture}
	...
	\subsection{Unitary time evolution and the Magnus expansion}
		...
		The Dyson series gives an explicit expression for \(\mathcal{U}(t, t_0)\) in terms of multiple time integrals over nested time commutators of \(H(t')\). % Cite 2? recent papers
	...
	\subsection{Rotating frame}
		...
		% \begin{align}
		% 	\mathcal{U}_k &= \begin{pmatrix}
		% 		e^{-i \omega_r \mathrm{D}t} & 0 & 0\\
		% 		0 & 1 & 0\\
		% 		0 & 0 & e^{i 2\pi f_r \mathrm{D}t}
		% 	\end{pmatrix} \mathcal{U}^r_k, \textrm{ for spin one, and}\\
		% 	\mathcal{U}_k &= \begin{pmatrix}
		% 		e^{-i \pi f_r \mathrm{D}t} & 0\\
		% 		0 & e^{i \pi f_r \mathrm{D}t}
		% 	\end{pmatrix} \mathcal{U}^r_k, \textrm{ for spin half.}
		% \end{align}
