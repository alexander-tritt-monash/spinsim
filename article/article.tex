...
\section{Implementation and architecture}
	...
	\subsection{Quantum mechanics background}
		...
		The $ J_x $, $ J_y $, $ J_z $ and $ Q $ are the only operators necessary to simulate many spin-one quantum systems, including quadratic Zeeman splitting described by the Breit-Rabi formula\cite{mockler_atomic_1961} important to experiments in our lab.
	...
	\subsection{Lie-Trotter based Exponentiator}
		...
		It can be avoided by replacing the exponentials and trigonometric functions along the diagonal with specialised implementations of functions such as $ \mathrm{expm}1(x) = \exp(x) - 1 $, designed for this purpose\cite{hewlett-packard_hp_1994}.
...
\section{Quality control}
	\subsection{Evaluation of accuracy}
		...
		This simulation involves continuously driving transitions in the system for $ 100\,\mathrm{ms} $, while exposing it to a short 1ms pulsed signal that the system should be able to sense.
	...
	\subsection{Comparison to alternatives}
		...
		Similarly to the internal \emph{spinsim} benchmarks\note{section???}, the expected spin projection was evaluated in each case, but only the states were compared to calculate a relative error via Equation \eqref{eq:error}.
	...
	\subsection{Parallelisation performance}
		...
		Another factor not shown in the plot is that, in practice, running highly code parallel code on a CPU on a personal computer will severely limit the responsiveness of other applications, as it can utilise the entire CPU (as it should).
