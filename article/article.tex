...
\section{Implementation and architecture}
	...
	\subsection{Exponentiator}
		...
		We choose $ n $ to be of the form $ n = 2^\tau $, as powers of two are particularly effient large numbers to raise matrices to.This can be done just by iterative squaring, which requires a small number of operations.
		...
		Instead, we tested the formula on $ 10^5 $ matrices for different values of $ \tau $, and compared them to results given by \texttt{scipy.linalg.expm()}, from the popular python library \emph{SciPy}.
	...
	\subsection{Discretisation and parallelisation}
		...
		\emph{spinsim} splits the full simulation into small time intervals $ [t_{k - 1}, t_{k}] $, and calculates time-evolution operators $ \mathcal{U}_k $ for these intervals in massive parallel on a GPU. % check phrase "massive"
		...
		To choose an appropriate value of $ \mathrm{D}t $, users must consider two factors.
		...
		Users should also make sure $ \mathrm{D}t $ represents a large enough sample rate for the state $ \psi_k $ or expected spin projection $ \left\langle \overrightarrow{J}\right\rangle_k $ to be used in their further analysis.
	...
	\subsection{Dynamically rotating frame}
		...
		This will yield a more accurate results when making discrete time steps by the integrator, as the distances stepped are smaller.
...
% \subsection{Software architecture}
	\subsection{Integrator architecture}
		\begin{figure}[h!]
			...
			\caption{The architecture of \emph{spinsim}}
			\label{fig:architecture}
		\end{figure}
		A visual summary of the architecture of \emph{spinsim} is shown in Figure \ref{fig:architecture}.
		...
		Within the loop, the integrator enters a device function (ie a GPU subroutine. All device functions are inline for speed) to sample the user-provided field functions $ \omega_i(t) $.
		After this, it makes applies the rotating frame transformation $R_{\omega_r}(t)$, before calculating the Gauss-Legendre weightings $ \overline{H}_1 $ and $ \overline{H}_2 $ in this frame. Next, the matrix exponentials are taken within another device function.
		...
		Once all threads have executed, using Equation \eqref{eq:integration_compilation}, the state $ \psi_k $ is calculated in a (CPU) \texttt{numba.jit()}ed function from the $ \mathcal{U}_k $ and an initial condition $ \psi_{\mathrm{init}} $.
