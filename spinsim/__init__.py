...
class Simulator:
    def __init__(self, get_field, spin_quantum_number, device = None, exponentiation_method = None, use_rotating_frame = True, integration_method = IntegrationMethod.MAGNUS_CF_4, trotter_cutoff = 28, max_registers = 63, threads_per_block = 64):
        """
        Parameters
        ----------
        ...
        device : :obj:`Device`
            The option to select which device will be targeted for integration. That is, whether the integrator is compiled for a CPU or GPU. See :obj:`Device` for all options.
        exponentiation_method : :obj:`ExponentiationMethod`
            Which method to use for matrix exponentiation in the integration algorithm. See :obj:`ExponentiationMethod` for all options.
        use_rotating_frame : :obj:`bool`
            Whether or not to use the rotating frame optimisation. If set to :obj:`True`, the integrator moves into a frame rotating in the z axis by an amount defined by the field in the z direction. This removes the (possibly large) z component of the field, which increases the accuracy of the output since the integrator will on average take smaller steps.
        ...
        try:
            self.compile_time_evolver(get_field, spin_quantum_number, device, use_rotating_frame, integration_method, exponentiation_method, trotter_cutoff, max_registers, threads_per_block)
        except:
            print("\033[31mspinsim error: numba could not jit get_field function into a device function.\033[0m\n")
            raise

    def compile_time_evolver(self, get_field, spin_quantum_number, device, use_rotating_frame = True, integration_method = IntegrationMethod.MAGNUS_CF_4, exponentiation_method = None, trotter_cutoff = 28, max_registers = 63, threads_per_block = 64):
        """
        Parameters
        ----------
        get_field : `function`
